...
def get_query(db, model_map):
    ...
    User = model_map['User']
    return db.session.query(User.id, Group.name.label(u'国家'),
                            User.name.label(u'名字'),
                            User.strength.label(u'武力'),
                            User.intelligience.label(u'智力'),
                            User.impact.label(u'影响力')).join(Group)
